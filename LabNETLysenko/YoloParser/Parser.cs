...
namespace YoloParser
{
    ...
    public class Parser
    {
        ...
        public async Task InitModelAsync()
        {
            ...
            if (!this.services.Exists(neuralNetName))
                using (HttpClient client = new())
                {
                    while (true)
                    {
                        try
                        {
                            services.Print("Downloading model...");
                            byte[] model = await client.GetByteArrayAsync(url);
                            services.WriteBytes(neuralNetName, model);
                            services.Print("Model succesfully downloaded!");
                        }
                        catch (Exception)
                        {
                            services.Print("Error!");
                            continue;
                        }
                        break;
                    }
        ...
        public async Task<List<ImageInfo>> AnalyzeAsync(string path, CancellationToken ctoken)
        {
            ...
            return await Task.Factory.StartNew(_ =>
            {
               ...
               (16.62, 10.52)
                ...
                {
                    var resultFileName = path.Split('\\').Last().Split('.').First() + $"{labels[obj.Class]}" + $"{objectCount}" + DateTime.Now.ToString("HH_mm_ss") + "Final.jpg";

                    result.Add(new ImageInfo(image.Clone(), resultFileName, obj.Class,
                        labels[obj.Class], obj.XMin, obj.YMin,
                        obj.XMax - obj.XMin, obj.YMax - obj.YMin, final, obj.Confidence));
                    objectCount++;
                }
                return result;
